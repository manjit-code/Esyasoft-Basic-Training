...
--3--
SELECT * FROM users u INNER JOIN orders o ON u.user_id = o.user_id 
INNER JOIN books b ON o.product_id = b.product_id WHERE o.product_id IN (102, 104);


--4--
select u.user_id, u.name, COUNT(*) FROM orders o INNER JOIN users u ON u.user_id=o.user_id GROUP BY u.user_id, u.name;

--5--
SELECT u.user_id, u.name, SUM(b.price) as total_spend FROM books b INNER JOIN orders o ON o.product_id = b.product_id
INNER JOIN users u ON u.user_id = o.user_id GROUP BY u.user_id, u.name;

--6--
SELECT TOP 1 u.user_id, u.name, SUM(b.price) as total_spend FROM books b INNER JOIN orders o ON o.product_id = b.product_id
INNER JOIN users u ON u.user_id = o.user_id GROUP BY u.user_id, u.name ORDER BY total_spend DESC;
